...
export class CreateAccountsEntriesTransactionsUsersTables1727576650100
  implements MigrationInterface
{
  name = 'CreateAccountsEntriesTransactionsUsersTables1727576650100'
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE "accounts" ("created_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), "updated_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), "deleted_at" TIMESTAMP WITH TIME ZONE, "id" character varying(24) NOT NULL, "type" text NOT NULL, "name" text, "balance_in_cents" integer NOT NULL, CONSTRAINT "PK_accounts_id" PRIMARY KEY ("id"))`
    )
    await queryRunner.query(
      `CREATE INDEX "IDX_accounts_name" ON "accounts" ("name") `
    )
    await queryRunner.query(
      `CREATE TABLE "users" ("created_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), "updated_at" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(), "deleted_at" TIMESTAMP WITH TIME ZONE, "id" character varying(24) NOT NULL, "email" character varying(255) NOT NULL, "wallet_account_id" character varying(24), CONSTRAINT "REL_users_wallet_account_id" UNIQUE ("wallet_account_id"), CONSTRAINT "PK_users_id" PRIMARY KEY ("id"))`
    )
    await queryRunner.query(
    ...
    await queryRunner.query(
      `ALTER TABLE "users" ADD CONSTRAINT "FK_users_wallet_account_id" FOREIGN KEY ("wallet_account_id") REFERENCES "accounts"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`
    )
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "users" DROP CONSTRAINT "FK_users_wallet_account_id"`
    )
    await queryRunner.query(
